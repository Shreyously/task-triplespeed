  ...
  try {
    return await withTx(async (client) => {
      const existingTx = await client.query(
        "select * from trade_transactions where buyer_id=$1 and idempotency_key=$2",
        [userId, idempotencyKey]
      );
      if (existingTx.rows[0]) {
        const tx = existingTx.rows[0];
        if (tx.listing_id !== listingId) throw new Error("Idempotency key already used for a different listing");
        return {
          listingId: tx.listing_id,
          buyerId: tx.buyer_id,
          gross: new Decimal(tx.gross_amount).toFixed(2),
          fee: new Decimal(tx.fee_amount).toFixed(2)
        };
      }
    ...
    try {
      ...
      if (!existingTx.rows[0]) throw err;
      const tx = existingTx.rows[0];
      if (tx.listing_id !== listingId) throw new Error("Idempotency key already used for a different listing");
      return {
        listingId: tx.listing_id,
